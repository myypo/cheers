# NOTICE

This project includes or adapts material from third-party projects. Third-party material remains under its original license. This notice is a project-wide attribution index and does not change the license terms of this project or of the works listed below.

Package-local notices and license texts are kept next to the code or skill material that uses them so independently distributed crates retain the relevant attribution.

## Impeccable

`skills/cheers-design` adapts material from Impeccable.

- Original work: <https://github.com/pbakaus/impeccable>
- Copyright: 2025-2026 Paul Bakaus
- License: Apache License 2.0
- Local notice: [`skills/cheers-design/NOTICE.md`](skills/cheers-design/NOTICE.md)
- License text: [`skills/cheers-design/LICENSES/impeccable-Apache-2.0.txt`](skills/cheers-design/LICENSES/impeccable-Apache-2.0.txt)

## Hypertext

Templating implementation code is derived from Hypertext.

- Original work: <https://github.com/vidhanio/hypertext>
- License: MIT
- Local notices:
  - [`crates/ast/NOTICE.md`](crates/ast/NOTICE.md)
  - [`crates/macros/NOTICE.md`](crates/macros/NOTICE.md)
- License texts:
  - [`crates/ast/LICENSES/hypertext-MIT.txt`](crates/ast/LICENSES/hypertext-MIT.txt)
  - [`crates/macros/LICENSES/hypertext-MIT.txt`](crates/macros/LICENSES/hypertext-MIT.txt)

## Datastar

`cheers` vendors and/or bundles Datastar runtime code.

- Original work: <https://github.com/starfederation/datastar>
- Copyright: © Star Federation
- License: MIT
- Local notice: [`crates/cheers/NOTICE.md`](crates/cheers/NOTICE.md)
- License text: [`crates/cheers/LICENSES/datastar-MIT.txt`](crates/cheers/LICENSES/datastar-MIT.txt)
- Vendored license copy: [`vendor/datastar/LICENSE.md`](vendor/datastar/LICENSE.md)

## Maudfmt

`cheers-fmt` contains formatter implementation code based on Maudfmt.

- Original work: <https://github.com/Jeosas/maudfmt>
- Copyright: 2025 Jean-Baptiste WINTERGERST
- License: MIT
- Local notice: [`crates/fmt/NOTICE.md`](crates/fmt/NOTICE.md)
- License text: [`crates/fmt/LICENSES/maudfmt-MIT.txt`](crates/fmt/LICENSES/maudfmt-MIT.txt)
