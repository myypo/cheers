...
fn generate_names_struct_and_impl(
    ...
    let struct_impl = quote! {
        impl #impl_generics #struct_ident #ty_generics #where_clause {
            /// Returns the form field-name bindings generated by `#[derive(Cheers)]`.
            #vis const fn form_names(&self) -> #form_names_ident {
                #form_names_ident {
...
pub(crate) fn generate_form_impl(item: &mut ItemStruct) -> Result<TokenStream, Error> {
    ...
    let names_struct_and_impl = if form_name_entries.is_empty() {
        TokenStream::new()
    } else {
        generate_names_struct_and_impl(
            vis,
            &item.ident,
            &item.generics,
            &ident_str,
