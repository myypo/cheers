...
export const morph = (
  ...
  if (
    (isHTMLOrSVG(oldElt) &&
      ...
      oldElt.hasAttribute(ignoreMorph) &&
      newContent.hasAttribute(ignoreMorph)) ||
    oldElt.parentElement?.closest(ignoreMorphAttr)
  ) {
    return;
  }
...
const morphNode = (
  ...
  if (type === 1 /* element type */) {
    ...
    if (oldElt.hasAttribute(ignoreMorph) && newElt.hasAttribute(ignoreMorph)) {
      return oldNode;
    }
    ...
    const preserveAttrs = (
      (newNode as HTMLElement).getAttribute(preserveAttr) ?? ""
    ).split(" ");
