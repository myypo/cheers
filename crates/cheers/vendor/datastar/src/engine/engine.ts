...
export const genRx = (
  ...
  // Rewrite signal roots into canonical bracket notation.
  // Any non-root path access must already use bracket syntax.
  // Examples:
  //   $count                         -> $['count']
  //   $count--                       -> $['count']--
  //   $demo['items']['user.123']     -> $['demo']['items']['user.123']
  //   $arr[$index]                   -> $['arr'][$['index']]
  //   $foo[obj.bar]                  -> $['foo'][obj.bar]
  //   $123                           -> $['123']

  const rewriteSignalRoot = (signalName: string) => `$['${signalName}']`;
        ...
        if (interpolationExpr !== undefined) {
          return `\${${interpolationExpr.replace(
            ...
            (_: string, innerSignalName: string) =>
              rewriteSignalRoot(innerSignalName),
          )}}`;
        }
        return rewriteSignalRoot(signalName);
      },
    )
    // [$x] -> [$['x']] ($ inside brackets)
    .replace(
